"use client";

import {
    Plus,
    File,
    Camera,
    X,
    ArrowRight,
    ChevronDown,
    MessagesSquare,
    Link,
    Pause,
} from "lucide-react";
import { useState, useRef, useCallback, type RefObject, useEffect } from "react";
import { Textarea } from "@/components/ui/textarea";
import { cn } from "@/lib/utils";
import { useAutoResizeTextarea } from "@/hooks/use-auto-resize-textarea";
import { useFileInput } from "@/hooks/use-file-input";
import { useClickOutside } from "@/hooks/use-click-outside";
import { useSidebarStore } from "@/lib/store";

const AI_MODELS = [
    { name: "GPT-4", description: "", color1: "bg-purple-800", color2: "bg-purple-200" },
    { name: "GPT-3.5", description: "", color1: "bg-blue-500", color2: "bg-blue-200" },
    { name: "Claude", description: "", color1: "bg-green-500", color2: "bg-green-200" },
];

const FileDisplay = ({
    fileName,
    onClear,
}: {
    fileName: string;
    onClear: () => void;
}) => (
    <div className="flex items-center gap-2 bg-black/5 dark:bg-white/5 w-fit px-3 py-1 rounded-lg">
        <File className="w-4 h-4 dark:text-white" />
        <span className="text-sm dark:text-white">{fileName}</span>
        <button
            type="button"
            onClick={onClear}
            className="ml-1 p-0.5 rounded-full hover:bg-black/10 dark:hover:bg-white/10"
        >
            <X className="w-3 h-3 dark:text-white" />
        </button>
    </div>
);

function BlinkingDot({ color1, color2 }: { color1: string, color2: string }) {
    return (
        <span className="relative flex items-center justify-center w-4 h-4">
            <span className={`absolute w-4 h-4 rounded-full ${color2}`} />
            <span className={`w-2 h-2 rounded-full ${color1} animate-blink`} />
        </span>
    );
}

interface AIInput10Props {
    onSendMessage: (text: string) => void;
    isAITyping?: boolean;
    onStopAIResponse?: () => void;
    shouldFocus?: boolean;
}

export default function AIInput_10({ 
    onSendMessage, 
    isAITyping = false, 
    onStopAIResponse,
    shouldFocus = false 
}: AIInput10Props) {
    const menuRef = useRef<HTMLDivElement>(null);
    const textareaRef = useRef<HTMLTextAreaElement>(null);
    const { toggleSidebar } = useSidebarStore();

    const [state, setState] = useState({
        value: "",
        fileName: "",
        selectedModel: "GPT-4",
        isMenuOpen: false,
        isModelMenuOpen: false,
    });

    // Auto focus when shouldFocus changes
    useEffect(() => {
        if (shouldFocus && !isAITyping && textareaRef.current) {
            textareaRef.current.focus();
        }
    }, [shouldFocus, isAITyping]);

    const { adjustHeight } = useAutoResizeTextarea({
        minHeight: 40,
        maxHeight: 200,
        ref: textareaRef
    });
    const { fileName, fileInputRef, handleFileSelect, clearFile } =
        useFileInput({ accept: "image/*", maxSize: 5 });

    const updateState = useCallback(
        (updates: Partial<typeof state>) =>
            setState((prev) => ({ ...prev, ...updates })),
        []
    );

    useClickOutside(menuRef as RefObject<HTMLElement>   , () => {
        if (state.isMenuOpen) updateState({ isMenuOpen: false });
        if (state.isModelMenuOpen) updateState({ isModelMenuOpen: false });
    });

    const handleSend = () => {
        if (state.value.trim() && !isAITyping) {
            onSendMessage(state.value.trim());
            updateState({ value: "" });
            adjustHeight(true);
        }
    };

    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        if (e.key === "Enter" && !e.shiftKey && !isAITyping) {
            e.preventDefault();
            handleSend();
        }
    };

    const selectedModelObj = AI_MODELS.find(m => m.name === state.selectedModel) || AI_MODELS[0];

    return (
        <div className="w-full py-4">
            <div className="rounded-xl bg-white dark:bg-black border border-black/10 dark:border-white/10">
                <div ref={menuRef}>
                    <div className="border-b border-black/10 dark:border-white/10">
                        <div className="flex justify-between items-center px-4 py-2 text-sm text-zinc-600 dark:text-zinc-400">
                            <div className="relative" data-model-menu>
                                <button
                                    type="button"
                                    onClick={() =>
                                        updateState({
                                            isModelMenuOpen:
                                                !state.isModelMenuOpen,
                                        })
                                    }
                                    className="flex items-center gap-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg px-2 py-1"
                                >
                                    <BlinkingDot color1={selectedModelObj.color1} color2={selectedModelObj.color2} />
                                    <span className="dark:text-white">
                                        {state.selectedModel}
                                    </span>
                                    <ChevronDown className="w-3 h-3 ml-0.5 dark:text-white" />
                                </button>

                                {state.isModelMenuOpen && (
                                    <div className="absolute bottom-full left-0 mb-1 w-64 bg-white dark:bg-zinc-800 rounded-md shadow-lg py-1 z-50 border border-black/10 dark:border-white/10">
                                        {AI_MODELS.map((model) => (
                                            <button
                                                type="button"
                                                key={model.name}
                                                className="w-full px-3 py-1.5 text-left hover:bg-black/5 dark:hover:bg-white/5 flex items-center gap-2 text-sm transition-colors dark:text-white"
                                                onClick={() =>
                                                    updateState({
                                                        selectedModel:
                                                            model.name,
                                                        isModelMenuOpen: false,
                                                    })
                                                }
                                            >
                                                <BlinkingDot color1={model.color1} color2={model.color2} />
                                                <span>{model.name}</span>
                                                <span className="text-xs text-zinc-500 dark:text-zinc-400">
                                                    {model.description}
                                                </span>
                                            </button>
                                        ))}
                                    </div>
                                )}
                            </div>

                            <button
                                type="button"
                                onClick={toggleSidebar}
                                className={cn(
                                    "flex items-center gap-2 p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400"
                                )}
                            >
                                <MessagesSquare className="w-4 h-4" />
                                <span>History</span>
                            </button>
                        </div>
                    </div>

                    {state.fileName && (
                        <div className="px-4 pt-2">
                            <FileDisplay
                                fileName={state.fileName}
                                onClear={() => {
                                    updateState({ fileName: "" });
                                    if (fileInputRef.current)
                                        fileInputRef.current.value = "";
                                }}
                            />
                        </div>
                    )}

                    <div className="relative px-2 py-2">
                        <div
                            className="absolute left-3 top-1/2 -translate-y-1/2"
                            data-action-menu
                        >
                            <button
                                type="button"
                                onClick={() =>
                                    updateState({
                                        isMenuOpen: !state.isMenuOpen,
                                    })
                                }
                                className="rounded-3xl bg-black/5 dark:bg-white/5 p-2 hover:bg-black/10 dark:hover:bg-white/10"
                            >
                                <Plus className="w-4 h-4 dark:text-white" />
                            </button>

                            {state.isMenuOpen && (
                                <div className="absolute left-0 bottom-full mb-2 bg-white dark:bg-zinc-800 rounded-md shadow-lg py-1 min-w-[140px] z-50 border border-black/10 dark:border-white/10">
                                    {[
                                        {
                                            icon: File,
                                            label: "Upload File",
                                            onClick: () =>
                                                fileInputRef.current?.click(),
                                        },
                                        {
                                            icon: Link,
                                            label: "Attach URL",
                                            onClick: () =>
                                                fileInputRef.current?.click(),
                                        },
                                    ].map(({ icon: Icon, label, onClick }) => (
                                        <button
                                            type="button"
                                            key={label}
                                            onClick={onClick}
                                            className="w-full px-3 py-1.5 flex items-center gap-2 hover:bg-black/5 dark:hover:bg-white/5 text-sm transition-colors dark:text-white"
                                        >
                                            <Icon className="w-4 h-4 dark:text-white" />
                                            <span>{label}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                        </div>

                        <Textarea
                            id="ai-input-10"
                            ref={textareaRef}
                            value={state.value}
                            placeholder="Type your message..."
                            disabled={isAITyping}
                            className={cn(
                                "w-full rounded-xl pl-14 pr-10 border-none resize-none bg-transparent dark:text-white placeholder:text-black/70 dark:placeholder:text-white/70",
                                "min-h-[40px] max-h-[100px] py-2.5",
                                isAITyping && "opacity-50 cursor-not-allowed"
                            )}
                            onKeyDown={handleKeyDown}
                            onChange={(e) => {
                                updateState({ value: e.target.value });
                                adjustHeight();
                            }}
                        />

                        <button
                            type="button"
                            onClick={isAITyping ? onStopAIResponse : handleSend}
                            disabled={isAITyping && !onStopAIResponse}
                            className={cn(
                                "absolute right-3 top-1/2 -translate-y-1/2 rounded-xl bg-black/5 dark:bg-white/5 p-1 hover:bg-black/10 dark:hover:bg-white/10",
                                isAITyping && !onStopAIResponse && "cursor-not-allowed"
                            )}
                        >
                            {isAITyping ? (
                                <Pause 
                                    className={cn(
                                        "w-4 h-4 dark:text-white",
                                        onStopAIResponse ? "text-red-500 animate-pulse" : "text-purple-500"
                                    )} 
                                />
                            ) : (
                                <ArrowRight
                                    className={cn(
                                        "w-4 h-4 dark:text-white",
                                        state.value ? "opacity-100" : "opacity-30"
                                    )}
                                />
                            )}
                        </button>
                    </div>
                </div>
            </div>
            <style jsx global>{`
                @keyframes blink {
                    0%, 80%, 100% { opacity: 1; }
                    40% { opacity: 0.3; }
                }
                .animate-blink {
                    animation: blink 1.4s infinite both;
                }
            `}</style>
        </div>
    );
}
